 ...
 * Provides the basic functionality while not tied to any specific audio
 * backend.
 */
class GenericSoundInstance : public SoundInstance {
 public:
  GenericSoundInstance(std::shared_ptr<SoundResource> resource,
                       float volume = 1.0f);
  ...
  [[nodiscard]] bool is_finished() override;
};
