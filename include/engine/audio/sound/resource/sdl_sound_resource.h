 ...
 * Inherits from the generic SoundResource class and contains SDL-specific audio
 * data.
 */
class SDLSoundResource : public SoundResource {
 public:
  SDLSoundResource(const std::string& name, const std::string& file_path,
                   SoundType type = SoundType::SDL_MIXER);
  ~SDLSoundResource() override = default;

  [[nodiscard]] const SDL_AudioSpec& audio_spec() const;
  [[nodiscard]] const Uint8* audio_buffer() const;
