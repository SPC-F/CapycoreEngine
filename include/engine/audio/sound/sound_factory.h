 ...
 * The reason for shared pointers is to manage the lifetime of sound resources
 * and instances easily share them across different parts of the audio system
...
class SoundFactory {
 public:
  ...
  static std::shared_ptr<SoundResource> create_sound_resource(
      const std::string& file_path, const std::string& name,
      SoundType type = SoundType::GENERIC);
  static std::unique_ptr<SoundInstance> create_sound_instance(
