...
/**
 * @brief Service responsible for managing game assets such as textures and
 * spritesheets.
 *
 * The AssetService provides the functionality to load sprite resources from
 * files, and register named textures and spritesheets and spritesheets in the
 * engine as well as retrieve them when needed.
 */
class AssetService : public IEngineService {
 public:
   ...
   * @return An optional reference to the vector of textures if found,
   * std::nullopt otherwise.
   */
  [[nodiscard]] std::optional<std::reference_wrapper<
      const std::vector<std::reference_wrapper<Texture>>>>
  try_get_spritesheet(const std::string& key) const;
   ...
   * @return A reference to the registered vector of textures representing the
   * spritesheet.
   * @throws An exception if the resource cannot be found or indices are out of
   * bounds.
   */
  std::reference_wrapper<const std::vector<std::reference_wrapper<Texture>>>
  create_spritesheet_for(const std::string& source, const std::string& name,
                         size_t from, size_t to);

  /**
   * @brief Attempts to retrieve a texture by its sprite name.
   * @param sprite The name of the registered texture resource.
   * @return An optional reference to the texture if found, std::nullopt
   * otherwise.
   */
  [[nodiscard]] std::optional<std::reference_wrapper<Texture>> try_get_texture(
   ...
   * @throws An exception if the resource cannot be found or index is out of
   * bounds.
   */
  ...
  /**
   * @brief Loads textures from a resource file and splits them into a grid of
   * sprites.
   ...
   */
  [[nodiscard]] std::optional<std::reference_wrapper<Font>> try_get_font(
   ...
   */
  [[nodiscard]] std::reference_wrapper<Font> register_font(
      const std::string& font_name, const std::string& font_path,
      int font_size);
 ...
 private:
  ...
  std::map<std::string, std::vector<std::reference_wrapper<Texture>>>
      named_assets_;
