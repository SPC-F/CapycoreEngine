 ...
 *
 * The Renderer class encapsulates the SDL rendering context and provides
 * methods to render game objects, clear the screen, and manage the rendering
 * window.
 */
class Renderer final : public IEngineService {
 private:
  ...
  // I tried just storing a Window& here, but that caused issues because the can
 ...
 public:
   ...
   * Clears the current rendering target with the drawing color set in the SDL
   * renderer.
   */
  void clear() const;
  Window& window();
