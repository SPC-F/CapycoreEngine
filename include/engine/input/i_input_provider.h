...
class IInputProvider {
 public:
  ...
  /** @brief Returns true on the frame the specified mouse button transitions to
   * pressed.
   *
  ...
  /**
   * @brief Returns the current mouse cursor position (X, Y) in window
   * coordinates.
   *
  ...
  // TODO: Replace std::pair<float, float> with a dedicated Vector2 type for
  // clearer semantics and math operations.
  /**
   * @brief Returns the change in mouse position since the last frame (deltaX,
   * deltaY).
   *
   * Can be used to detect relative movement for camera rotation or drag
   * operations.
   */
   ...
   *
   * Useful for conditional logic that only triggers when the cursor changes
   * position.
   */
   ...
   *
   * Can be used to determine the general scroll direction (up, down, left,
   * right).
   */
