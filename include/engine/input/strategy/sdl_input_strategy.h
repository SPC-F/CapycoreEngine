...
class SDLInputStrategy : public IInput {
 public:
  void register_events(std::map<KeyCode, KeyState>& key_states,
                       MouseState& mouse_state) override;
 ...
 private:
   ...
   *
   * This method maps an SDL mouse button (e.g., SDL_BUTTON_LEFT) to a
   * platform-independent MouseButton enum value, enabling consistent mouse
   * button handling across different platforms.
   */
