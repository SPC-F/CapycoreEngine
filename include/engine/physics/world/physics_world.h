...
class PhysicsWorld {
 public:
   ...
   */
  static BodyDistance2D distance(const Body2DTransform& a,
   ...
   * box body with multiple box fixtures will calculate the distance based on
   * the closest fixture.
   *
   * @param a The first body's transform.
   * @param b The second body's transform.
   ...
   * fixtures.
   */
  static BodyDistance2D fixture_distance(const Body2DTransform& a,
                                         const Body2DTransform& b);

  [[nodiscard]] b2WorldId world_id() const noexcept;

  [[nodiscard]] int32_t velocity_iterations() const noexcept;
  int32_t velocity_iterations(int32_t iterations) noexcept;

  [[nodiscard]] float gravity_x() const noexcept;
 ...
 private:
   ...
   * of its world instances. Meaning a pointer or reference cannot be used here
   * without more complex management.
   */
  b2WorldId world_id_{};
