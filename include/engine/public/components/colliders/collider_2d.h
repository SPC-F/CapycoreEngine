...
/**
 * @brief A 2D collider base that defines the shape and physical properties of a
 * GameObject for collision detection.
 * @note This is an abstract base class and should be inherited by specific
 * collider shapes like CircleCollider2D, BoxCollider2D, etc.
 *
...
class Collider2D : public Renderable {
 ...
 private:
  ...
  std::vector<std::function<void(Collider2D&, Collider2D&)>>
      on_trigger_exit_actions_;
  std::vector<std::function<void(Collider2D&, Collider2D&)>>
      on_collision_enter_actions_;
  std::vector<std::function<void(Collider2D&, Collider2D&)>>
      on_collision_exit_actions_;
};