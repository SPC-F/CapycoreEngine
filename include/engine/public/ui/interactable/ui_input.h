 ...
 *
 * UIInput represents a text input field in the user interface. It inherits from
 * UIInteractable and adds properties and behaviors specific to text input, such
 * as handling focus, text content, and text color.
 */
class UIInput : public UIInteractable {
 public:
  UIInput(Scene& scene, float width, float height, Point pivot, Point anchor,
          const std::string& placeholder = "", const std::string& font = "",
