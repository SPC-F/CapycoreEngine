 ...
 public:
  ...
  /**
    ...
    The overloaded struct is a trick that basically boils down to the following:
        "Be a structure that overloads the call type for anything that is inside
    of you".
...
*/
  ...
  T get_value_or_default(const std::string &key, T default_value) {
    // So we provide the overloads for the () operator here. Every version of it
    // takes in x and returns T or throws...
    return std::visit(overloaded{
                          [](const T value) -> T { return value; },
                          [](auto) -> T {
                            throw std::invalid_argument{
                                "invalid value for T in settings::value_of"};
                          },
                      },
