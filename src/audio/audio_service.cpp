...
std::optional<std::shared_ptr<SoundResource>> AudioService::register_sound(
    const std::string& file_path, const std::string& name, SoundType type) {
...
bool AudioService::unregister_sound(const std::string& name) {
  active_instances_.erase(
      std::remove_if(active_instances_.begin(), active_instances_.end(),
                     [&name](const std::unique_ptr<SoundInstance>& instance) {
                       return instance && instance->resource() &&
...
std::optional<std::shared_ptr<SoundResource>> AudioService::get_sound_resource(
    const std::string& file_path, const std::string& name) const noexcept {
  auto file_it = std::find_if(sound_resources_.begin(), sound_resources_.end(),
                              [&file_path](const auto& pair) {
                                return pair.second &&
                                       (pair.second->file_path() == file_path);
                              });
...
std::reference_wrapper<SoundInstance> AudioService::play_sound(
    std::shared_ptr<SoundResource> sound_resource, float volume, bool loop) {
  if (!sound_resource) {
...
std::reference_wrapper<SoundInstance> AudioService::play_sound(
    const std::string& name, float volume, bool loop) {
...
void AudioService::stop_sound(SoundInstance& instance) {
  auto it =
      std::find_if(active_instances_.begin(), active_instances_.end(),
...
bool AudioService::has_sound_instance(const std::string& name) const noexcept {
  return std::any_of(active_instances_.begin(), active_instances_.end(),
                     [&name](const std::unique_ptr<SoundInstance>& instance) {
                       return instance && instance->resource() &&
                              (instance->resource()->name() == name);
                     });
...
std::optional<std::reference_wrapper<SoundInstance>>
AudioService::get_sound_instance(const std::string& name) const {
  auto it =
      std::find_if(active_instances_.begin(), active_instances_.end(),
                   [&name](const std::unique_ptr<SoundInstance>& instance) {
                     return instance && instance->resource() &&
...
std::vector<std::reference_wrapper<SoundInstance>>
AudioService::get_all_playing_sounds() const {
