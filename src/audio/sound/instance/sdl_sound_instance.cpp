...
void SDLSoundInstance::play() {
  ...
  if (spec.format == SDL_AUDIO_S16LE || spec.format == SDL_AUDIO_S16BE) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      samples[i] = static_cast<Sint16>(
          std::clamp(scaled, -sample_clamp_range, sample_clamp_range - 1));
    }
  }
  ...
  SDL_PutAudioStreamData(stream_.get(), temp.data(),
...
bool SDLSoundInstance::is_paused() const noexcept { return paused_; }
