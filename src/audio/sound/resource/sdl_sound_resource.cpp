...
SDLSoundResource::SDLSoundResource(const std::string& name,
                                   const std::string& file_path, SoundType type)
  ...
  if (file_path.size() < 4 ||
      file_path.substr(file_path.size() - 4) != ".wav") {
    throw std::runtime_error("SDLSoundResource only supports .wav files: " +
                             file_path);
  }
...
const SDL_AudioSpec& SDLSoundResource::audio_spec() const { return spec_; }

const Uint8* SDLSoundResource::audio_buffer() const {
  return audio_buffer_.get();
}

Uint32 SDLSoundResource::length() const { return audio_length_; }