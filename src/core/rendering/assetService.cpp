...
AssetService::try_get_spritesheet(const std::string& key) const {
  if (const auto it = named_assets_.find(key); it != named_assets_.end()) {
    // const ref, not c ref. constructor functions for these things are called
    // ref and cref respectively.
...
std::reference_wrapper<const std::vector<std::reference_wrapper<Texture>>>
AssetService::create_spritesheet_for(const std::string& source,
                                     const std::string& name, size_t from,
                                     size_t to) {
  const auto maybe_resource = try_get_spritesheet(source);

  if (!maybe_resource.has_value()) {
    throw std::invalid_argument(std::format(
        "The given argument for {} has no associated resource registered.",
        source));
  }

  const std::vector<std::reference_wrapper<Texture>>& resource =
      *maybe_resource;
  ...
  named_assets_.emplace(name,
                        std::vector(resource.begin() + static_cast<int>(from),
...
std::vector<std::reference_wrapper<Texture>> AssetService::load_from_resource(
    const std::string& file, const std::string& name, int rows, int cols) {
  const std::string file_path = AssetService::resource_path + file;

  const auto& renderer_service =
      Engine::instance().services->get_service<RenderingService>().get();

  SDL_Renderer* renderer = renderer_service.renderer_->sdl_renderer_.get();
  ...
  auto& resource =
      named_assets_.at(name);  // <-- reference to actual stored vector
...
std::reference_wrapper<Texture> AssetService::register_texture(
    const std::string& resource_name, const std::string& texture_name,
    const size_t index) {
  ...
  if (index >= resource.size()) {
    throw std::invalid_argument(
        std::format("Invalid range for creating texture subset"));
  }

  auto texture = resource.at(index);
  named_assets_.emplace(texture_name, std::vector{texture});
...
std::reference_wrapper<Texture> AssetService::get_default_texture() {
  ...
  SDL_Texture* tex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
                                       ...
                                       white_texture_size, white_texture_size);

  if (!tex) {
    throw std::runtime_error("Failed to create white texture");
  }

  std::vector<Uint32> pixels(static_cast<size_t>(white_texture_size) *
                                 static_cast<size_t>(white_texture_size),
                             white_pixel);

  SDL_UpdateTexture(tex, nullptr, pixels.data(),
                    static_cast<int>(static_cast<size_t>(white_texture_size) *
                                     sizeof(Uint32)));
  SDL_SetTextureBlendMode(tex, SDL_BLENDMODE_BLEND);

  auto new_texture = std::unique_ptr<Texture>(new Texture(tex));
  textures_.emplace_back(std::move(new_texture));
  named_assets_.emplace("default", std::vector<std::reference_wrapper<Texture>>{
                                       *textures_.back()});
...
std::optional<std::reference_wrapper<Font>> AssetService::try_get_font(
    const std::string& font_name, int font_size) const {
...
std::reference_wrapper<Font> AssetService::register_font(
    const std::string& font_name, const std::string& font_path, int font_size) {
  const std::string key = font_name + "_" + std::to_string(font_size);

  std::unique_ptr<TTF_Font, void (*)(TTF_Font*)> font =
      std::unique_ptr<TTF_Font, void (*)(TTF_Font*)>(
  ...
  auto font_ptr = std::unique_ptr<Font>(
      new Font(font_name, static_cast<int8_t>(font_size), font.release()));
