...
Color SdlImageStrategy::get_default_image_color(
    SDL_Texture*
...
void SdlImageStrategy::set_image_color(
    const Color& color,
    SDL_Texture*
        texture) {  // NOLINT [readability-convert-member-functions-to-static]
...
void SdlImageStrategy::draw(Component& component, Camera& camera) {
  ...
  if (!parent_opt.has_value()) {
    throw std::runtime_error(
        "Cannot draw Image component without a parent GameObject");
  }
  ...
  SDL_RenderTextureRotated(
      &sdl_renderer_, texture_ptr, nullptr, &target, transform.rotation(),
