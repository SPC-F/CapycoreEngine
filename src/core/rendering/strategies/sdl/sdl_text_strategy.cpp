...
std::reference_wrapper<TTF_Font> SdlTextStrategy::get_font(
    const std::string& name, const std::string& path, int font_size) {
  auto& asset_service =
...
void SdlTextStrategy::draw_cached(Text& text, const UIObject& ui_object,
                                  const Transform& transform, float scale_x,
                                  float scale_y) {
  ...
  float y = transform.position().y + ((ui_object.height() - last_font_height_) *
                                      default_transform_divider);

  x += text.offset().x;
  y += text.offset().y;

  SDL_FRect dst{x * scale_x, y * scale_y, last_font_width_ * scale_x,
                last_font_height_ * scale_y};

  SDL_RenderTextureRotated(&sdl_renderer_, texture_.get(), nullptr, &dst,
                           transform.rotation(), nullptr, SDL_FLIP_NONE);
}

void SdlTextStrategy::draw_fresh(Text& text, const UIObject& ui_object,
                                 const Transform& transform, float scale_x,
                                 float scale_y) {
  auto& font =
      SdlTextStrategy::get_font(text.font(), text.font_path(), text.font_size())
          .get();
  SDL_Color color{
      static_cast<Uint8>(text.color().r), static_cast<Uint8>(text.color().g),
      static_cast<Uint8>(text.color().b), static_cast<Uint8>(text.color().a)};
  ...
  std::unique_ptr<SDL_Surface, decltype(&SDL_DestroySurface)> surf(
      TTF_RenderText_Blended(&font, text.text().c_str(), text.text().length(),
                             color),
      SDL_DestroySurface);
