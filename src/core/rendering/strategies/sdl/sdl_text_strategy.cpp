...
void SdlTextStrategy::draw(Component& component) {
    ...
    if (!text->dirty() && texture_) {
        ...
        // Center text over UIObject
        SDL_FRect dst{
            transform.position().x + (ui_object.width() - last_font_width_) * 0.5f * scale_x,
            transform.position().y + (ui_object.height() - last_font_height_) * 0.5f * scale_y,
    ...
    SDL_Surface* surf = TTF_RenderText_Blended(&font, text->text().c_str(), text->text().length(), color);
