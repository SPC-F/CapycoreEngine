...
bool InputSystem::is_key_held(KeyCode key) const {
  ...
  return it != key_states_.end() && it->second.current;
}

bool InputSystem::is_key_pressed(KeyCode key) const {
  auto it = key_states_.find(key);
  return it != key_states_.end() && it->second.current && !it->second.previous;
}
...
bool InputSystem::any_key_pressed() const {
  return std::ranges::any_of(key_states_, [](const auto& kv) {
    return kv.second.current && !kv.second.previous;
  });
}
...
bool InputSystem::is_mouse_held(MouseButton button) const {
  ...
  return it != mouse_state_.buttons.end() && it->second.current;
}
...
bool InputSystem::any_mouse_held() const {
  return std::ranges::any_of(mouse_state_.buttons,
                             [](const auto& kv) { return kv.second.current; });
}

bool InputSystem::any_mouse_pressed() const {
  return std::ranges::any_of(mouse_state_.buttons, [](const auto& kv) {
    return kv.second.current && !kv.second.previous;
  });
}
