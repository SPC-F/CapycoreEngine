...
Client::Client(std::reference_wrapper<Router> router) : router_(router) {
  const ENetAddress* address = nullptr;
  size_t peer_count = 1;
  size_t channel_limit = 2;
  enet_uint32 max_bandwidth = 0;

  client_ = enet_host_create(address, peer_count, channel_limit, max_bandwidth,
                             max_bandwidth);
...
void Client::connect(const std::string& host_ip, int connection_port) {
  ...
  server_peer_ = enet_host_connect(client_, &address, channel_limit, data);

  if (server_peer_ == nullptr)
    throw std::runtime_error(
        "ENet could not create a peer for connection attempt.");
...
void Client::disconnect() noexcept {
  ...
  Message message{
      serialize_message(body, DefaultMessageTypes::CLIENT_DISCONNECT)};
  send(message);
}
...
std::string Client::get_uuid() const noexcept { return local_uuid_; }

std::string Client::get_host_ip() const noexcept { return host_ip_; }
...
void Client::register_on_disconnect_handler() noexcept {
  ...
  router_.get().register_handler(DefaultMessageTypes::HOST_DISCONNECT,
                                 std::move(handler));
}
...
void Client::register_on_snapshot_full_handler() noexcept {
  ...
  router_.get().register_handler(
      MessageType(DefaultMessageTypes::SNAPSHOT_FULL), std::move(handler));
}
...
void Client::register_on_snapshot_delta_handler() noexcept {
  ...
  router_.get().register_handler(
      MessageType(DefaultMessageTypes::SNAPSHOT_DELTA), std::move(handler));
}
