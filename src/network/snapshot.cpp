...
static void serialize_object(const GameObject& go, const NetworkIdentity& net,
 ...
 *
 * Iterates through all game objects in the scene, filters them using the
 * provided predicate, and serializes those that pass the filter into a single
 * message payload.
 *
 * @tparam FilterFunc A callable type (lambda, function pointer, functor) that
 * filters NetworkIdentity objects. Expected signature: bool(const
 * NetworkIdentity&) Return true to include an object in the snapshot, false to
 * exclude it.
 *
 * @param scene The scene containing game objects to snapshot.
 * @param messageType The type of message to create (e.g., FULL_SNAPSHOT or
 * DELTA_SNAPSHOT).
 * @param filter A callable that determines which NetworkIdentity objects to
 * include in the snapshot.
 *
 * @return A Message containing the serialized snapshot of all objects that pass
 * the filter predicate.
 */
template <typename FilterFunc>
static Message create_snapshot(const Scene& scene,
  ...
  payload.reserve(sizeof(count) +
                  objects.size() * (UUID_LEN + sizeof(uint16_t) * 2));
...
static std::optional<std::reference_wrapper<GameObject>> find_or_create_object(
    Scene& scene, const std::string& uuid_str,
  ...
  if (offset + sizeof(uint16_t) > payload.size()) return std::nullopt;
  ...
  if (peek_off + prefab_id_len > payload.size()) return std::nullopt;

  std::string prefab_type_id(
      reinterpret_cast<const char*>(payload.data() + peek_off), prefab_id_len);

  auto& engine = Engine::instance();
  auto& prefab_service = engine.services->get_service<PrefabService>().get();
  if (!prefab_service.has_prefab(prefab_type_id)) return std::nullopt;
...
Message create_full_snapshot(const Scene& scene,
                             DefaultMessageTypes messageType) {
  return create_snapshot(scene, messageType,
                         [](const NetworkIdentity&) { return true; });
}

Message create_delta_snapshot(const Scene& scene,
                              DefaultMessageTypes messageType) {
  auto msg = create_snapshot(
      scene, messageType,
      [](const NetworkIdentity& net) { return net.is_dirty(); });
...
// Advances the offset over a serialized GameObject payload without constructing
// one.
static void skip_gameobject_payload(const std::vector<uint8_t>& data,
                                    size_t& offset) {
...
void apply_delta_snapshot(Scene& scene, const Message& msg) {
  ...
  for (uint16_t i = 0; i < count; ++i) {
    if (offset + UUID_LEN + sizeof(uint16_t) > payload.size()) break;
    ...
    auto obj_opt =
        find_or_create_object(scene, std::string(uuidbuf), payload, offset);
