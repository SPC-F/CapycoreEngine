...
PhysicsService::PhysicsService() {
  ...
  physics_raycaster_ =
      std::make_unique<PhysicsRaycaster>(physics_world_->world_id());
  physics_creation_factory_ =
      std::make_unique<PhysicsCreationFactory>(physics_world_->world_id());
}

void PhysicsService::update(
    float dt, const std::vector<std::reference_wrapper<GameObject>>& objects) {
