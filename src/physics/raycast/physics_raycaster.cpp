...
PhysicsRaycaster::PhysicsRaycaster(b2WorldId world_id) : world_id_(world_id) {}
...
ColliderRayResult PhysicsRaycaster::raycast_closest(const b2Vec2& origin,
  ...
  b2RayResult ray =
      b2World_CastRayClosest(world_id_, origin, translation, filter);
  ...
  if (ray.hit) {
    ...
    collision_result.distance =
        ray.fraction * std::sqrt((translation.x * translation.x) +
...
std::vector<ColliderRayResult> PhysicsRaycaster::raycast_all(
    const b2Vec2& origin, const b2Vec2& translation, b2QueryFilter filter) {
  std::vector<b2RayResult> results;

  auto raycast_callback = [](b2ShapeId shape_id, b2Vec2 point, b2Vec2 normal,
  ...
  b2World_CastRay(world_id_, origin, translation, filter,
                  static_cast<b2CastResultFcn*>(raycast_callback), &results);
...
std::vector<ColliderRayResult> PhysicsRaycaster::raycast_filtered(
    const b2Vec2& origin, const b2Vec2& translation, uint16_t category_mask) {
  b2QueryFilter filter;
  filter.categoryBits = all_categories;
...
ColliderRayResult PhysicsRaycaster::raycast_segment(const b2Vec2& start,
  ...
  b2RayResult ray =
      b2World_CastRayClosest(world_id_, start, translation, filter);
