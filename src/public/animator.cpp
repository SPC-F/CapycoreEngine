...
std::vector<std::reference_wrapper<Texture>> get_frames(const std::string& sprite_sheet_name) {
    ...
    if (!sprite_sheet.has_value()) {
        throw std::runtime_error("Animator: Sprite sheet not found: " + sprite_sheet_name);
    }
...
Animator::Animator(const std::string& sprite_sheet_name, const int interval_ms)
    ...
    interval_ms_(interval_ms > 0 ? interval_ms : throw std::invalid_argument("Animator: interval_ms must be greater than 0")),
...
int Animator::calculate_next_frame_index(const int intervals_advanced) const {
    ...
    if (current_texture_index_ < 0 || current_texture_index_ >= animation_size) {
        throw std::out_of_range("next_frame_index: current_frame_index out of range");
    }
    ...
    return mathematically_correct_modulo(current_texture_index_ + intervals_advanced, animation_size);
}
...
void Animator::update_sprite_texture(const int new_frame_index) {
    ...
    if (!sprite.has_value()) {
        throw std::runtime_error("Animator: Parent GameObject does not have a Sprite component.");
    }
...
void Animator::update(const float dt_seconds) {
    ...
    const int actual_dt_ms = static_cast<int>(dt_seconds * 1000.0f) + accumulator_time_ms_;
    ...
    // How many frames have we advanced? Take of the excess time if any, be left with only an exact number of frames.
    const int intervals_advanced = static_cast<int>((actual_dt_ms - accumulator_time_ms_) / interval_ms_);
