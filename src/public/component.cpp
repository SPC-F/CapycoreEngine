...
bool Component::active() const noexcept { return active_; }
...
const std::optional<std::reference_wrapper<GameObject>>& Component::parent()
    const noexcept {
  return parent_;
}
...
void Component::remove_on_attach(size_t index) {
  if (index < on_attach_actions_.size()) {
    on_attach_actions_.erase(std::next(on_attach_actions_.begin(),
...
void Component::remove_on_detach(size_t index) {
  if (index < on_detach_actions_.size()) {
    on_detach_actions_.erase(std::next(on_detach_actions_.begin(),
                                       static_cast<std::ptrdiff_t>(index)));
  }
}
