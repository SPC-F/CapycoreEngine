...
AudioSource::AudioSource(std::string audio_file_path, bool play_on_awake,
                         bool loop, float volume)
    : audio_file_path_(std::move(audio_file_path)),
      play_on_awake_(play_on_awake),
      loop_(loop),
...
std::shared_ptr<SoundResource> AudioSource::get_or_register_resource() {
  ...
  audio_name_ =
      filename + "_" + std::to_string(reinterpret_cast<uintptr_t>(this));

  auto resource_opt =
      audio_service.get_sound_resource(audio_file_path_, audio_name_);
  if (!resource_opt) {
    resource_opt = audio_service.register_sound(audio_file_path_, audio_name_,
                                                SoundType::SDL_MIXER);
  }
...
void AudioSource::play(bool loop) {
  ...
  if (!resource) {
    throw std::runtime_error(
        "Failed to get or register sound resource for AudioSource");
  }
...
float AudioSource::volume() const noexcept { return volume_; }

void AudioSource::volume(float value) noexcept { volume_ = value; }

bool AudioSource::loop() const noexcept { return loop_; }

void AudioSource::loop(bool value) noexcept { loop_ = value; }

bool AudioSource::play_on_awake() const noexcept { return play_on_awake_; }

void AudioSource::play_on_awake(bool value) noexcept { play_on_awake_ = value; }

std::string AudioSource::audio_name() const noexcept { return audio_name_; }
