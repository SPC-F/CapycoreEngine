...
Collider2D::Collider2D(float friction, float bounciness, Point offset)
    : friction_(friction), bounciness_(bounciness), offset_(offset) {
    ...
    if (auto gameobject_opt = parent(); gameobject_opt.has_value()) {
      ...
      if (!gameobject.get_component<Rigidbody2D>().has_value()) {
        throw std::runtime_error(
            "Collider2D requires a Rigidbody2D component on the same "
            "GameObject.");
      }
    }
  });
...
std::reference_wrapper<Rigidbody2D> Collider2D::get_rigidbody() {
  ...
  if (!rigidbody_opt.has_value()) {
    throw std::runtime_error(
        "No Rigidbody2D component found on the parent GameObject.");
  }

  return rigidbody_opt->get();
}

float Collider2D::friction() const noexcept { return friction_; }
...
float Collider2D::bounciness() const noexcept { return bounciness_; }
