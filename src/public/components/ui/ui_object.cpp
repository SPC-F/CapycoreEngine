...
UIObject::UIObject(float width, float height, Point pivot, Point anchor,
                   Scene& scene)
    : GameObject(scene),
      width_(width),
      height_(height),
...
float UIObject::height() const { return height_; }
...
Point UIObject::pivot() const { return pivot_; }
...
Point UIObject::anchor() const { return anchor_; }
