...
UIInteractable::UIInteractable(Scene& scene, float width, float height,
                               Point pivot, Point anchor)
    : UIObject(scene, width, height, pivot, anchor), interaction_state_() {}

UIInteractionState& UIInteractable::state() { return interaction_state_; }
...
bool UIInteractable::in_range(const Point& mouse_pos) const {
  ...
  return (pos.x <= mouse_pos.x && mouse_pos.x <= pos.x + width() &&
          pos.y <= mouse_pos.y && mouse_pos.y <= pos.y + height());
}

void UIInteractable::disable() { interaction_state_.disabled = true; }

void UIInteractable::enable() { interaction_state_.disabled = false; }
