...
std::optional<SimpleStorage::StorageValue> SimpleStorage::try_get(
    const StorageKey& key_name) {
  if (const auto it = session_storage_.find(key_name);
      it != session_storage_.end()) {
    return it->second;
  }
  return std::nullopt;
}
...
void SimpleStorage::delete_all() { session_storage_.clear(); }
...
void SimpleStorage::load() {
  ...
  while (std::getline(in, line)) {
    ...
    try {
      ...
      std::cerr << "[SimpleStorage] Error: invalid int on line " << line_number
                << ": " << val << "\n";
    }
  }
}
