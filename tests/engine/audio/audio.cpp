...
TEST_CASE(
    "audio_service_register_should_return_resource_when_registering_valid_"
    "sound",
    "[AudioService]") {
  auto file_path =
      std::string(PROJECT_ROOT) + "/resources/sounds/bonetrousle.wav";

  AudioService audio_service;
  auto sound_name = "Bonetrousle";

  auto resource =
      audio_service.register_sound(file_path, sound_name, SoundType::SDL_MIXER);
  REQUIRE(resource != nullptr);

  SECTION("registering_same_sound_name_returns_existing_resource") {
    auto second = audio_service.register_sound(file_path, sound_name,
                                               SoundType::SDL_MIXER);
...
TEST_CASE("audio_service_unregisters_sound_resources_correctly",
          "[AudioService]") {
  auto file_path =
      std::string(PROJECT_ROOT) + "/resources/sounds/bonetrousle.wav";
  ...
  AudioService audio_service;
  auto sound_name = "Bonetrousle";
  audio_service.register_sound("./resources/sounds/bonetrousle.wav", sound_name,
                               SoundType::SDL_MIXER);
  ...
  SECTION("unregistering_existing_sound_removes_resource_and_instances") {
    ...
    REQUIRE(audio_service.get_sound_resource(file_path, sound_name) ==
            std::nullopt);
  }
...
TEST_CASE("audio_service_plays_and_stops_sounds", "[AudioService]") {
  ...
  audio_service.register_sound("./resources/sounds/bonetrousle.wav", name,
                               SoundType::SDL_MIXER);
...
TEST_CASE("audio_service_reports_and_retrieves_sound_instances",
          "[AudioService]") {
  AudioService audio_service;
  auto name = "Bonetrousle";
  audio_service.register_sound("./resources/sounds/bonetrousle.wav", name,
...
TEST_CASE("audio_service_handles_invalid_inputs_safely", "[AudioService]") {
  ...
  SECTION("play_sound_throws_on_null_resource") {
    REQUIRE_THROWS_AS(audio_service.play_sound("", 0.5f, false),
                      std::invalid_argument);
  }

  SECTION("stop_sound_handles_null_unique_ptr_gracefully") {
    std::reference_wrapper<SoundInstance> null_instance =
        *static_cast<SoundInstance*>(nullptr);
