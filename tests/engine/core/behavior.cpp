...
TEST_CASE("behavior_runs_on_awake", "[Behavior]") {
  class TestBehavior : public Behavior {
   public:
    void on_awake() override { awake_called = true; }
  ...
  auto& behavior_script = game_object.add_component<BehaviorScript>(
      std::make_unique<TestBehavior>());

  REQUIRE(behavior_script.enabled() == true);
  REQUIRE(
      dynamic_cast<TestBehavior*>(&behavior_script.behavior())->awake_called ==
      true);
}
...
TEST_CASE("behavior_on_start_and_update", "[Behavior]") {
  class TestBehavior : public Behavior {
   public:
    ...
    void on_update(float dt) override { update_called = true; }
    void on_destroy() override {}
  ...
  REQUIRE(
      dynamic_cast<TestBehavior*>(&behavior_script.behavior())->start_called ==
      true);
  REQUIRE(
      dynamic_cast<TestBehavior*>(&behavior_script.behavior())->update_called ==
      false);
...
TEST_CASE("behavior_on_destroy_called_on_destruction", "[Behavior]") {
  ...
  class TestBehavior : public Behavior {
   public:
    ...
    void on_start() override {}
    void on_update(float dt) override {}
    void on_destroy() override { on_destroy_callback_(*this); }
  };
...
TEST_CASE("behavior_can_access_game_object_and_transform", "[Behavior]") {
  ...
  REQUIRE(dynamic_cast<TestBehavior*>(&behavior_script.behavior())
              ->game_object()
              .id() == game_object.id());
  REQUIRE(
      &dynamic_cast<TestBehavior*>(&behavior_script.behavior())->transform() ==
