...
TEST_CASE("AddGameObject_AddsGameObjectToScene_WhenExistingGameObject",
          "[Scene]") {
  // arrange
  const std::string& name = "Test Scene";
  auto scene_service = SceneService();
  ...
  REQUIRE(scene.get_game_object(game_object.id()).get().id() ==
          game_object.id());
}
...
TEST_CASE("AddGameObjects_AddsMultipleGameObjectsToScene", "[Scene]") {
  ...
  REQUIRE(scene.get_game_object(game_object_1_id).get().id() ==
          game_object_1_id);
  REQUIRE(scene.get_game_object(game_object_2_id).get().id() ==
          game_object_2_id);
}
...
// Do not make these tests. They will call the engine singleton and give you a
// big headache.
