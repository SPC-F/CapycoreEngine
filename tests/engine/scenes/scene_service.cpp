...
TEST_CASE("AddScene_WhenSceneExists_ReturnsFoundScene", "[SceneService]") {
  ...
  // assert
  REQUIRE(&dummy ==
          &scene);  // same reference, but are these value or address equal?
}
...
TEST_CASE("LoadScene_MovesDontDestroyOnLoadObjects_ToNewScene",
          "[SceneService]") {
  // arrange
  auto scene_service = SceneService();
...
TEST_CASE("LoadScene_MovesDontDestroyOnLoadObject_WithChildren",
          "[SceneService]") {
  // arrange
  auto scene_service = SceneService();
  Scene& scene1 = scene_service.add_scene("Scene1");
...
TEST_CASE("LoadScene_DoesNotMoveChildMarkedDontDestroy_IfParentIsNotMarked",
          "[SceneService]") {
  // arrange
  auto scene_service = SceneService();
  Scene& scene1 = scene_service.add_scene("Scene1");
...
// Do not make these tests. They will work with the engine and kick off SDL
// windows. You will be hurt, do not.
...
// TEST_CASE("AddSceneAndLoad_WhenSceneNotExist_LoadsAndAddsScene",
// "[SceneService]")