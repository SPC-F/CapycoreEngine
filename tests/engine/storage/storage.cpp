...
TEST_CASE("try_get_returns_empty_optional_when_key_missing",
          "[SimpleStorage]") {
...
TEST_CASE("save_writes_file_and_load_restores_int_float_string",
          "[SimpleStorage][Persistence]") {
...
TEST_CASE("save_then_load_preserves_overwritten_values",
          "[SimpleStorage][Persistence]") {
  auto& storage = TestableStorage::instance();
  storage.delete_all();
...
TEST_CASE("load_handles_escaped_strings", "[SimpleStorage][Persistence]") {
  ...
  REQUIRE(std::get<std::string>(*storage.try_get("escaped")) ==
          "hello \"world\"");
}
